...
internal object PromptTemplates {
        ...
        - Use this exact outer schema:
          {
            ...
            "problem": {
              ...
              "starterCode": "...",
              "hints": ["..."],
              "submissionTestSuite": { ... optional ... },
              "executionPipeline": "single_method|encode_decode_round_trip|serialize_deserialize_round_trip|operation_sequence"
            }
          }
        - Always return the full problem object, not only changed fields
        - Use empty strings or empty arrays instead of omitting the core fields
        - For exampleInput, use assignment-style input like `nums = [1,2,3]` and `target = 9`
        - Do not use `Input:` labels or `name: value` formatting for exampleInput
        - Keep starterCode as a plain JSON string, not markdown fences
        - If submissionTestSuite or executionPipeline are uncertain, leave them out
        - For summary, write one short sentence saying the problem was generated by the current AI model and naming the problem type when you can
    ...
    val solve = """
        ...
        - Use the provided custom tests, local submission tests, and recorded run results when they are available
        - If there are recorded failing cases, call them out and make sure the final approach handles them
        - If the current draft already passes the recorded cases, say that briefly before generalizing
